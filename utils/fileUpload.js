...
function fileUpload() {
    const methods = {
        ...
        upload: multer({
            fileFilter: function(req, file, cb) {
                try {
                    ...
                    if ((file.mimetype == 'image/png' || file.mimetype == 'image/jpeg' || file.mimetype == 'image/jpg' || file.mimetype == 'image/gif') && lIntFileSize <= 5) {
                        cb(null, true)
                    } else {
        ...
        s3GetSignedURL: async(key, folderName) => {
            try {
                console.log(await s3.getSignedUrl("getObject", {
